...
1. 在终端执行`export EVA_API_KEY=你的deepseek API key`

EVA支持OpenAI接口形式的LLM，可以是Ollma、vLLM拉起的本地模型，也可以是DeepSeek、OpenAI等官网API。因此，你可以直接修改代码中的`EVA_BASE_URL`, `EVA_MODEL_NAME`, `EVA_API_KEY`变量为对应的服务，也可以选择从环境变量导入，其中linux是export命令设置环境变量，windows是set命令。
